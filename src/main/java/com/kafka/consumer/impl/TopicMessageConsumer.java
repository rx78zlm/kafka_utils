 ...
 */
@SPI("topic")
public class TopicMessageConsumer<V> extends AbstractMessageConsumer<V> {
    ...
    Properties initConnectProperties(String topicName) {
        ...
        if (!properties.containsKey(ConsumerPropDesc.GROUP_ID.getCfgName())) {
            // 没有group.id，默认生成一个
            properties.put(ConsumerPropDesc.GROUP_ID.getCfgName(), topicName + "_topic_" + NetUtils.getLocalAddress().getHostAddress());
        }
